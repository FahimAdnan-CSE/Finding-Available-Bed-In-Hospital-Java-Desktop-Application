...
public class CodeOperations {
    ...
    public static void updateHospitalInfo(String personName,String hospitalAddress, String personImagePath, int idStr, JFrame frame){
        try{
            Connection mySqlConn = CodeSQLConnection.getConnection();
            String mySqlQuery = " update hospital_details set hospital_name = ?, address = ?,hospital_logo = ? where id =? ";
            PreparedStatement preparedStatement = mySqlConn.prepareStatement(mySqlQuery);
            ...
            preparedStatement.setInt(4,idStr);
            //preparedStatement.setString(6, idStr);
            try{
                preparedStatement.execute();
                JOptionPane.showMessageDialog(frame, "Record Has Been Saved!");
    ...
    public static void hospitalJList(JList list,JFrame frame){        
        ...
        try{
            ...
            while(resultSet.next()){
                ...
                listModel.addElement(new CodeHospitalInfo(resultSet.getInt("id"),resultSet.getString("hospital_name"),resultSet.getString("address"),resultSet.getString("total_icu"),
        ...
        list.setCellRenderer(new CodeCustomRenderer());
    ...
    public static void searchIDHospitalData(String searchQuery,JLabel pendingStatus,JList list,JFrame frame){
        DefaultListModel listModel = new DefaultListModel();
        try{
            Connection sqlConn = CodeSQLConnection.getConnection();
            String query = searchQuery;
            PreparedStatement preparedStatement = sqlConn.prepareStatement("select * from hospital_details where id like '%"+query+"%' && status = 1 ");
            ResultSet resultSet = preparedStatement.executeQuery();
            if(resultSet.isBeforeFirst()){
                while(resultSet.next()){
                    byte[] hospitalImageByte = resultSet.getBytes("hospital_logo");
                ...
                pendingStatus.setText("<html><p style=\"font-size:15px;\"><b> Hospital Found for ID:></b>"+query);
            }else{
                pendingStatus.setText("<html><p style=\"font-size:15px;\"><b>"+searchQuery+" Already Approved.</b>");
            }            
        }catch(Exception exception){
